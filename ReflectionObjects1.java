...
public class ReflectionObjects1 {
	...
	public static void main(String[] args){
		try	//Wise to use try-catch block to catch exceptions at runtime
		{
			...
			{	
				...
				provided the return type of the previous matches the parameter type of the next. A method which takes no parameters
				is not checked for this condition. And when a method which returns a void, the value is null, as such
				the value is given a new assignment of 1.*/
				...
				for(Iterator<Method> iter = totalMethods.iterator();iter.hasNext();)
				{
					...
					if(!nextMethod.equals(invokeMethod)) // A method will not be chained with itself
					{
						...
						/*Approach 2: wherein a method is chained only if the return type of the previous method
								matches the parameter type of the next*/
						
						else if((parameter[0].equals(Integer.TYPE) && (value instanceof Integer)) || 
								(parameter[0].equals(Float.TYPE) && (value instanceof Float)) || 
								(parameter[0].equals(Character.TYPE) && (value instanceof Character)) ||
								(parameter[0].equals(java.lang.Number.class) && (value instanceof Integer || value instanceof Float)))
						{
